...
class UMAPDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        edges_to_exp = edges_to_exp + torch.empty(edges_to_exp.shape).normal_(mean=0,std=0.0005)
        edges_from_exp = self.data[self.edges_from_exp[index]]
        edges_from_exp = edges_from_exp + torch.empty(edges_from_exp.shape).normal_(mean=0,std=0.0005)
