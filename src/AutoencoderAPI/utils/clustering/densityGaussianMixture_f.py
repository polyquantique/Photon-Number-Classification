...
from .multiGaussianFit import fit_gaussian
...
class density_gaussianMixture():
    ...
    def __init__(self, X_low, 
        ...
        f = fit_gaussian(self.bins, self.density)

        self.cluster_means = f.cluster_means
        self.cluster_variance = f.cluster_variance
        self.cluster_weights = f.cluster_weights
        self.mins = torch.tensor(f.mins).to(self.device)
        ...
        for label, _ in enumerate(self.cluster_weights):
    ...
    def plot_cross_talk(self, zeros_number=0):
        ...
        for i in range(num_distributions):
            for j in range(num_distributions):
                ...
                distr1 = self.gaussian_function(w, self.cluster_means[i], self.cluster_variance[i], self.cluster_weights[i])
                distr2 = self.gaussian_function(w, self.cluster_means[j], self.cluster_variance[j], self.cluster_weights[j])
                ...
                integral1 = quad(self.gaussian_function, x1, x2, args=(self.cluster_means[i], self.cluster_variance[i], self.cluster_weights[i]))
                integral2 = quad(self.gaussian_function, x3, x4, args=(self.cluster_means[j], self.cluster_variance[j], self.cluster_weights[j]))
                integral3 = quad(self.gaussian_function, -1, 1, args=(self.cluster_means[i], self.cluster_variance[i], self.cluster_weights[i]))
    ...
    def plot_density(self, get_data=False):
        ...
        with plt.style.context(self.style_name):
            ...
            for means, variance, weights  in zip(self.cluster_means, self.cluster_variance, self.cluster_weights):
                gaussian_ = gaussian_ + self.gaussian_function(x, means, variance, weights).flatten()
                plt.plot(x, self.gaussian_function(x.reshape(-1,1), means, variance, weights), color="k")
    ...
    def plot_cluster(self,
        ...
        with plt.style.context(self.style_name):
            ...
            gaussian_ = np.zeros(len(x))
            for means, variance, weights  in zip(self.cluster_means, self.cluster_variance, self.cluster_weights):
                gaussian_ = gaussian_ + self.gaussian_function(x, means, variance, scaling*weights).flatten()
                plt.plot(x, self.gaussian_function(x.reshape(-1,1), means, variance, scaling*weights), color="k")
