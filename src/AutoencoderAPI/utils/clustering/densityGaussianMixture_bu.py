...
class density_gaussianMixture():
    ...
    def __init__(self, X_low, 
        ...
        maxs = maxs_init[np.exp(kd.score_samples(maxs_init.reshape(-1,1))) > min_cluster_prob]
        ...
        bounds_list2 = []
        for index in range(len(maxs)):
            guess_list.append([maxs[index], weights_init[index], np.abs(mins[1]-mins[0]/3)])
            bounds_list1.append([self.min_, 0, self.min_])
            bounds_list2.append([self.max_, 1, self.max_])

        guess = np.ravel(guess_list) 
        bounds = (np.ravel(bounds_list1), np.ravel(bounds_list2))

        popt, pcov = curve_fit(self.func_gaus, self.bins, self.density, p0=guess, maxfev = 10000, bounds=bounds)
        
        #plt.figure()
        #plt.plot(self.bins, self.func_gaus(self.bins, *popt))
        #plt.show()
        
        self.cluster_means = popt[0::3]
        self.weights = popt[1::3]
        self.cluster_covariance = popt[2::3]
        """
    ...
    def plot_cross_talk(self, zeros_number=0):
        ...
        if zeros_number != 0:
            ...
            scaling = self.weights * self.number_sample/total
